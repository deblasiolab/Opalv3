...
public class ExactCountAligner_Time extends ExactCountAligner {
	...
	protected void initialize () {		
		...
		Alignment alA = Alignment.buildNewAlignment(revA, A.seqIds, true);
		Alignment alB = Alignment.buildNewAlignment(revB, B.seqIds, true);
		...
		if (Aligner.useStructure) { // structure is stored in forward orientation. This is easiest way to get correct score calculated
			alignedSeqs = SequenceConverter.buildReverseAlignment(al.getAlignment().seqs);
		} else {
			alignedSeqs = al.getAlignment().seqs;
		}
				
		costUpperBound = Aligner.calcCost(alignedSeqs,A.K,B.K,idsAB);
		...
		if (A.seqs.length * B.seqs.length > Aligner.linearCutoff)
			shapeTester = new ShapeTesterLinear (costUpperBound, al.getD(), al.getH(), al.getV());
		else
			shapeTester = new ShapeTesterQuadratic (costUpperBound, al.getD(), al.getH(), al.getV());
		...
		long pessimisticUpperBound = Aligner.calcCost(al.getAlignment().seqs,A.K,B.K, idsAB);
		...
		dpRows[currRow][0].add(new ShapeQuadratic()); // initialize entry (0,0) with the flush shape and cost 0	  
	}	
