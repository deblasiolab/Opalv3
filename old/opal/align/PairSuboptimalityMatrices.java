...
public class PairSuboptimalityMatrices {
	...
	public PairSuboptimalityMatrices (Alignment A, Alignment B) { // single sequence
		this(A,B,true);
	}
	
	public PairSuboptimalityMatrices (Alignment A, Alignment B, boolean compress) { // single sequence
		this.A = A;
		this.C = B;
		...
		Alignment Arev = Alignment.buildNewAlignment(SequenceConverter.buildReverseAlignment(A.seqs), A.seqIds);
		Alignment Brev = Alignment.buildNewAlignment(SequenceConverter.buildReverseAlignment(B.seqs), B.seqIds);
		...
		result = SequenceConverter.convertPathToCharAlignment(forwardAligner.getPath(), SequenceConverter.convertIntsToSeqs(A.seqs), SequenceConverter.convertIntsToSeqs(B.seqs));
...
//		}
		...
		forwardAligner.H[M][P] -= Aligner.gammaTerm/2;
		forwardAligner.V[M][P] -= Aligner.gammaTerm/2;
		backwardAligner.H[M][P] -= Aligner.gammaTerm/2;
		backwardAligner.V[M][P] -= Aligner.gammaTerm/2;
...
//					nearlyOptimal_vertStart[0] = 0;
				...
				long best = bigNumber ;
				if (i>0 && j>0) {
					int sigma = Aligner.costs[A.seqs[0][i-1]][C.seqs[0][j-1]]; 
					best =  forwardAligner.D[i][j] + backwardAligner.D[M-(i-1)][P-(j-1)] - sigma;
				}
				if (j>0)
					best = Math.min(best, forwardAligner.H[i][j] + backwardAligner.H[M-i][P-(j-1)] - ((i==0||i==M)?Aligner.lambdaTerm:Aligner.lambda));
				if (i>0)
					best = Math.min(best, forwardAligner.V[i][j] + backwardAligner.V[M-(i-1)][P-j] - ((j==0||j==P)?Aligner.lambdaTerm:Aligner.lambda));
...
//					if (nearlyOptimal_vertStart[j] == -1) nearlyOptimal_vertStart[j] = i;
		...
		for (int j=0; j<P; j++) {
			long best =            forwardAligner.D[M][j] + Aligner.lambdaTerm * (P-j) + Aligner.gammaTerm;
			best = Math.min(best, forwardAligner.H[M][j] + Aligner.lambdaTerm * (P-j) + Aligner.gammaTerm/2);
			best = Math.min(best, forwardAligner.V[M][j] + Aligner.lambdaTerm * (P-j) + Aligner.gammaTerm + Aligner.gamma/2);
			if (best <= optCost + delta) {
...
//				if (nearlyOptimal_vertStart[j] == -1) nearlyOptimal_vertStart[j] = M;
		...
		for (int i=0; i<M; i++) {
			long best =            forwardAligner.D[i][P] + Aligner.lambdaTerm * (M-i) + Aligner.gammaTerm;
			best = Math.min(best, forwardAligner.H[i][P] + Aligner.lambdaTerm * (M-i) + Aligner.gammaTerm + Aligner.gamma/2);
			best = Math.min(best, forwardAligner.V[i][P] + Aligner.lambdaTerm * (M-i) + Aligner.gammaTerm/2);
			if (best <= optCost + delta) {
...
//		if (nearlyOptimal_vertStart[P] == -1) nearlyOptimal_vertStart[P] = M;
	...
	public int getSubsSubopt(int i,int k) {
		 ...
		 * 
		 */	
		int sigma = Aligner.costs[A.seqs[0][i-1]][C.seqs[0][k-1]]; 
	...
	private int getExtSubopt1(int i,int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			...
			best =  forward.H[j][k-forward.shift[j]] + backward.H[N-j][P-(k-1)-backward.shift[N-j]] - ((j==0||j==N)?Aligner.lambdaTerm:Aligner.lambda);
		}
	...
	private int getExtSubopt2(int i,int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			...
			best =  forward.V[i][k-forward.shift[i]] + backward.V[M-(i-1)][P-k-backward.shift[M-(i-1)]] - ((k==0||k==P)?Aligner.lambdaTerm:Aligner.lambda);
		} else {
			int j = i; int N=M; // just to sync the code below with the graph above. In other words: read "i" as "j" in the right graph above
			//k > 0   ... from function call
			if (j==N && k==P) {
				best = Math.min(forward.D[N][P-forward.shift[N]], forward.H[N][P-forward.shift[N]] + Aligner.gammaTerm/2);
			} else if (j==0 && k==0){
				best = Math.min(backward.D[N][P-backward.shift[N]], backward.H[N][P-backward.shift[N]] + Aligner.gammaTerm/2);
			} else if (k==0 || k==P) {
				/*
				...
				if (j<N ) {					
					...
					best = Math.min(best,forward.H[j][k-forward.shift[j]] + (i==0?Aligner.gammaTerm:Aligner.gamma)/2 + backward.D[N-j][P-k-backward.shift[N-j]]);
					if (j>0) {
						best = Math.min(best, forward.D[j][k-forward.shift[j]] + backward.D[N-j][P-k-backward.shift[N-j]]);
						best = Math.min(best, forward.D[j][k-forward.shift[j]] + Aligner.gamma/2 + backward.H[N-j][P-k-backward.shift[N-j]]);
					}
				} else {  //j==N , k<P
					//diag:horiz  at bottom boundary			
					best = forward.D[N][k-forward.shift[N]] + Aligner.gammaTerm/2 + backward.H[0][P-k-backward.shift[0]];
					//	horiz:horiz
	...
	public int getOpenSubopt1(int i,int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			...
			if (j>0) 
				best =  forward.D[j][k-1-forward.shift[j]] + (j==N?Aligner.gammaTerm:Aligner.gamma)/2 + backward.H[N-j][P-(k-1)-backward.shift[N-j]] ;
			else 
				best =  forward.H[0][k-forward.shift[0]] + backward.H[N][P-(k-1)-backward.shift[N]] - Aligner.lambdaTerm ;
		}
	...
	public int getOpenSubopt2(int i,int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			int gamma_open = (k==P ? Aligner.gammaTerm :Aligner.gamma)/2;
			if (i>1) {
				//check this at boundaries
				best =  forward.D[i-1][k-forward.shift[i-1]] + gamma_open + backward.V[M-(i-1)][P-k-backward.shift[M-(i-1)]];
			} else { //if ( i==1 ) {
				best = forward.V[1][k-forward.shift[1]] + backward.V[M][P-k-backward.shift[M]] - (k==0||k==P?Aligner.lambdaTerm:Aligner.lambda);
			}
		} else {
			int j = i; int N=M; // just to sync the code below with the graph above. In other words: read "i" as "j" in the right graph above
			best = bigNumber;
			...
			if (j==0 /*k<P from calling function*/) { // should really only be run if i==1
				int sigma = Aligner.costs[A.seqs[0][1-1]][C.seqs[0][(k+1)-1]];
				best = forward.D[1][k+1-forward.shift[1]] + backward.D[N][P-k-backward.shift[N]] -  sigma;
				best = Math.min(best, forward.H[0][k+1-forward.shift[0]] + backward.H[N][P-k-backward.shift[N]] - Aligner.lambdaTerm);
			} else if (j==N /*k>0 from calling function*/) {
				int sigma = Aligner.costs[A.seqs[0][N-1]][C.seqs[0][k-1]];
				best = forward.D[N][k-forward.shift[N]] + backward.D[0][P-(k-1)-backward.shift[0]] - sigma;
			} else { // (k<P from calling function)
				best = forward.D[j][k-forward.shift[j]] + Aligner.gamma/2 + backward.H[N-j][P-k-backward.shift[N-j]] ;
				best = Math.min(best, forward.D[j][k-forward.shift[j]] +  backward.D[N-j][P-k-backward.shift[N-j]]);
			}
		}
	...
	public int getOpenSubopt_special(int i, int k_prime, int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			...
			int ext =  (i==1?Aligner.lambdaTerm : Aligner.lambda) * ext_cnt;
			int close =  (i==1?Aligner.gammaTerm : Aligner.gamma)/2;
			...
			int ext =  (j==N ? Aligner.lambdaTerm : Aligner.lambda) * ext_cnt;
			int open = (j==N ? Aligner.gammaTerm :Aligner.gamma)/2;
	...
	public int getCloseSubopt1(int i,int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			...
			if (j<N ) {
				best =  forward.H[j][k-forward.shift[j]] + (j==0?Aligner.gammaTerm:Aligner.gamma)/2 + backward.D[N-j][P-k-backward.shift[N-j]] ;
			} else { // j==N; only makes sense when i==M, so calling function filters for that.
				best = forward.H[N][k-forward.shift[N]] + backward.H[0][P-(k-1)-backward.shift[0]] - Aligner.lambdaTerm ;
			}			
	...
	public int getCloseSubopt2(int i,int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			if (i<M) {
				best =  forward.V[i][k-forward.shift[i]] + (k==0?Aligner.gammaTerm:Aligner.gamma)/2 + backward.D[M-i][P-k-backward.shift[M-i]];
			} else { //if ( i==M ) {     k==P only if i==M
				best =  forward.V[M][k-forward.shift[M]] + backward.V[1][P-k-backward.shift[1]] - (k==0||k==P?Aligner.lambdaTerm:Aligner.lambda);
			}
		} else {
			int j = i; int N=M; // just to sync the code below with the graph above. In other words: read "i" as "j" in the right graph above
			best = bigNumber;
			if (j==0 ) {  //ok for k to be 0
				...
				best = Math.min(best, forward.H[N][k-forward.shift[N]] +  backward.H[0][P-(k-1)-backward.shift[0]] - Aligner.lambdaTerm);
			} else { // 0<j<N , 0<k<P
				best = forward.D[j][k-forward.shift[j]] + backward.D[N-j][P-k-backward.shift[N-j]] ;
				best = Math.min(best, forward.H[j][k-forward.shift[j]] + Aligner.gamma/2 + backward.D[N-j][P-k-backward.shift[N-j]]);
			}	
		}
	...
	public int getCloseSubopt_special(int i, int k_prime, int k, boolean isLeftGraph) {
		...
		if (isLeftGraph) {
			//k_prime > 0  and  i > 0 ... from function call
			int ext_cnt = k - k_prime - 2;
			int ext =  (i==M?Aligner.lambdaTerm : Aligner.lambda) * ext_cnt;
			int close =  (i==M?Aligner.gammaTerm : Aligner.gamma)/2;
			...
			int ext_cnt = k - k_prime - 1;
			int ext =  (j==0 ? Aligner.lambdaTerm : Aligner.lambda) * ext_cnt;
			int open = (j==0 ? Aligner.gammaTerm :Aligner.gamma)/2;
	...
	public int getHExtSubopt(int i,int j) {
		...
		long best =  forward.H[i][j-forward.shift[i]] + backward.H[M-i][P-(j-1)-backward.shift[M-i]] - ((i==0||i==M)?Aligner.lambdaTerm:Aligner.lambda);				
	...
	public int getHOpenSubopt(int i,int j) {
		...
		long best=0;
		if (j==1) {
			best =  forward.H[i][1-forward.shift[i]] + backward.H[M-i][P-backward.shift[M-i]] - ((i==0||i==M)?Aligner.lambdaTerm:Aligner.lambda);
		} else {
			best =  forward.D[i][j-1-forward.shift[i]] + backward.H[M-i][P-(j-1)-backward.shift[M-i]] + (i==M?Aligner.gammaTerm:Aligner.gamma);;
		}
	...
	public int getHCloseSubopt(int i,int j) {
		...
		long best=0;
		if (j==P) {
			best =  forward.H[i][P-forward.shift[i]] + backward.H[M-i][1-backward.shift[M-i]] - ((i==0||i==M)?Aligner.lambdaTerm:Aligner.lambda);
		} else {
			best =  forward.H[i][j-forward.shift[i]] + backward.D[M-i][P-j-backward.shift[M-i]] + (i==0?Aligner.gammaTerm:Aligner.gamma);;
		}
	...
	public int getVExtSubopt(int i,int j) {
		...
		long best =  forward.V[i][j-forward.shift[i]] + backward.V[M-(i-1)][P-j-backward.shift[M-(i-1)]] - ((j==0||j==P)?Aligner.lambdaTerm:Aligner.lambda);				
	...
	public int getVOpenSubopt(int i,int j) {
		...
		long best;
		if (i==1) {
			best =  forward.V[1][j-forward.shift[1]] + backward.V[M][P-j-backward.shift[M]] - ((j==0||j==P)?Aligner.lambdaTerm:Aligner.lambda);
		} else {
			best =  forward.D[i-1][j-forward.shift[i-1]] + backward.V[M-(i-1)][P-j-backward.shift[M-(i-1)]] + (j==P?Aligner.gammaTerm:Aligner.gamma);
		}
	...
	public int getVCloseSubopt(int i,int j) {
		...
		long best;
		if (i==M) {
			best =  forward.V[M][j-forward.shift[M]] + backward.V[1][P-j-backward.shift[1]] - ((j==0||j==P)?Aligner.lambdaTerm:Aligner.lambda);
		} else {
			best =  forward.V[i][j-forward.shift[i]] + backward.D[M-i][P-j-backward.shift[M-i]] + (j==0?Aligner.gammaTerm:Aligner.gamma);
		}
