...
//		ShutdownHandler.shutdown(1);
	...
	protected void fillTable() {
		...
		for (int i=1; i<=M; i++){
			for (int j=1; j<=N; j++){
					if (i==1 || j==1) {
						gamma_close_tmp = gammaTerm/2;
					} else {
						gamma_close_tmp = gamma/2;
					}	
					gamma_h_open_tmp = (i==M?gammaTerm:gamma)/2;
					gamma_v_open_tmp = (j==N?gammaTerm:gamma)/2;
				...
				H[i][j] += (i==M ? Aligner.lambdaTerm : Aligner.lambda); 
				V[i][j] += (j==N ? Aligner.lambdaTerm : Aligner.lambda); 
		...
		while (i>0 && j>0) {					
			...
			if (Direction.diag != dir) {
				if ( (Direction.horiz == dir && i==M) || (Direction.vert == dir && j==N) ) {
					gamma_open_tmp = gammaTerm/2;
				} else {
					gamma_open_tmp = gamma/2;		
				}
			}
			if (i==1 || j==1) {
				gamma_close_tmp = gammaTerm/2;
			...
			if (Direction.horiz == dir){
				int ext = (i==M?Aligner.lambdaTerm : Aligner.lambda);
				if ( cost == H[i][j-1] + ext )				
				...
				int ext = (j==N?Aligner.lambdaTerm : Aligner.lambda);
				...
				int sub = costs[A.seqs[0][i-1]][B.seqs[0][j-1]];
