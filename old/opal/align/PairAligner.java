	...
	protected void fillTable() {
		...
		for (int i=1; i<=M; i++){
			for (int j=1; j<=N; j++){
					...
					if (useStructure) {
						structGamma_tmpA = Math.round(getStructGapOpenModiferPair(A.seqIds[0], i-1));
						structGamma_tmpB = Math.round(getStructGapOpenModiferPair(B.seqIds[0], j-1));
					}
				...
				//substitution cost
				D[i][j] += costs[A.seqs[0][i-1]][B.seqs[0][j-1]];

				if (useStructure) {
					D[i][j] += Math.round(getStructSubModifierPair(A.seqIds[0], B.seqIds[0], i-1, j-1));
					V[i][j] += Math.round(getStructGapExtModiferPair(A.seqIds[0], i-1));
					H[i][j] += Math.round(getStructGapExtModiferPair(B.seqIds[0], j-1));
				}
			}
		}
	}
...
//			heatmap.setPointHeat(j, i, 4);
			...
			if (Direction.horiz == dir){
				if (useStructure) base += Math.round(getStructGapExtModiferPair(B.seqIds[0], j-1));								
				...
				if (useStructure) base += Math.round(getStructGapExtModiferPair(A.seqIds[0], i-1));
				...
				base = costs[A.seqs[0][i-1]][B.seqs[0][j-1]];
				if (useStructure) base += Math.round(getStructSubModifierPair(A.seqIds[0], B.seqIds[0], i-1, j-1));
