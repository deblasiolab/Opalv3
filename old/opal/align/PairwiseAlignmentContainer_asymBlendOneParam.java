...
public class PairwiseAlignmentContainer_asymBlendOneParam extends
	...
	public PairwiseAlignmentContainer_asymBlendOneParam(Tree tree, float[][] distances) {
		super(tree, distances);
	}

	protected void setBlendParams (int neighborCnt) {
		c_weight = neighborCnt==0? 0 : consistency_weight/neighborCnt;
		normalizer = 1/(PairSuboptimalityMatrices.delta * Aligner.gammas.length *(1+ neighborCnt==0? 0 : consistency_weight)); // will make the worse-case cost just double the cost of an unmodified edge	
	}
