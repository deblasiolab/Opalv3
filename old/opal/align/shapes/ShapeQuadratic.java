			...
			for (int j = 0; j < L; j++) {
				if (Aligner.Direction.horiz == dir)  {
					if (seqBlocks[i] >= seqBlocks[j+K]  &&  SequenceConverter.GAP_VAL != B.seqs[j][b-1]) {
						cost += (aPos < A.firstLetterLoc[i] || aPos >= A.lastLetterLoc[i]) ? Aligner.gammaTerm : Aligner.gamma;
						if (Aligner.useStructure && aPos > 0) {
							int pos = ((StructureAlignment)A).origSeqIndices[i][aPos-1];
							if (pos>0)
								cost += StructureAlignment.gapOpenMods[ StructureFileReader.structureNeighborLevels[A.seqIds[i]][pos] ] ;
						}
					}
				} else if (Aligner.Direction.vert == dir)  {
					if (seqBlocks[j+K] >= seqBlocks[i]  &&  SequenceConverter.GAP_VAL != A.seqs[i][a-1]) {
						cost += (bPos < B.firstLetterLoc[j] || bPos >= B.lastLetterLoc[j]) ? Aligner.gammaTerm : Aligner.gamma;					
						if (Aligner.useStructure && bPos > 0) {
							int pos = ((StructureAlignment)B).origSeqIndices[j][bPos-1];
							if (pos>0)
								cost += StructureAlignment.gapOpenMods[ StructureFileReader.structureNeighborLevels[B.seqIds[j]][pos] ] ;
						}
					}
