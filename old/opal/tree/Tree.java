...
//	public Tree (int[][] seqs, Distance dist, Aligner al) {
	public Tree (Alignment[] alignments, Aligner al, Distance distance, int currIteration, int verbosity) {	
		dist = distance;
		defaultAligner = al;
	...
	protected void initializeStructures (int K, Alignment[] alignments, int currIteration) {
		for (int i=0; i<K; i++) {
			...
			if (currIteration<iterations || AlignmentMaker.initAlignmentProvided) {
				...
				Alignment tmpAl;
				tmpAl = Alignment.buildNewAlignment(Alignment.getDegappedCopy(tmp)[0], i);
...
//				LogWriter.stdErrLog(i + "," + j + ": " );
				...
				if (currIteration == iterations && !AlignmentMaker.initAlignmentProvided) {
					...
					int[] ids = {i,j};
					Alignment tmpAl; 
					tmpAl = Alignment.buildNewAlignment(tmp, ids);
		...
		if (verbosity > 0) LogWriter.stdErrLogln("\n Pairwise alignments complete.");
