...
public class AlignmentMaker_SuboptimalityTester extends AlignmentMaker {
	...
	public void initialize (String file, String structFile) { 
		...
		seqs = SequenceConverter.convertSeqsToInts( chars );				
		K = names.length;
		this.file = file;
	}
	...
	final public int[][] buildAlignment ( String costName, int verbosity, boolean toUpper) {
		...
		for (int i=0; i<K-1; i++ ) {
			alA = Alignment.buildNewAlignment(seqs[i], i);
			charsA[0] = chars[i];
			namesA[0] = names[i];
			for (int j=i+1; j<K; j++){
				alB = Alignment.buildNewAlignment(seqs[j], j);
				charsB[0] = chars[j];
				namesB[0] = names[j];

				LogWriter.stdOutLogln(names[i] + ", " + names[j] + " :");
				cont = new PairSuboptimalityMatrices (alA, alB);
				
				if (verbosity>0) {
					char[][] result = SequenceConverter.convertPathToCharAlignment (cont.getAlignment().getPath(), charsA, charsB);
					AlignmentWriter wr;
					if ( AlignmentWriter.outFormat == OutputType.fasta)
						wr = new FastaWriter(namesA, namesB, result, 1, 1, toUpper);
					else {//CLUSTAL 
						wr = new ClustalWriter(namesA, namesB, result, 1, 1, toUpper);
