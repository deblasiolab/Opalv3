...
public class AlignmentMaker_SingleSequences extends AlignmentMaker {
	...
	final public void buildTree ( String costName, int verbosity) { 
		...
		if (treeOutFile != null ){
			...
			if (verbosity>0) LogWriter.stdErrLogln("output tree file = " + treeOutFile);
		}
...
//		if (Tree.distanceType == DistanceType.kmer_normcost && currIteration<Tree.iterations) {
		if ((Tree.distanceType == DistanceType.kmer_normcost || Tree.distanceType == DistanceType.normcost) && currIteration<Tree.iterations) {
			...
			maker.initialize(seqs, names);
			int[][] res = maker.buildAlignment(costName, verbosity, toUpper); 
			alignments = new Alignment[1];
			
			int[] ids = new int[res.length];
			for (int i=0; i<res.length; i++) ids[i] = i;
			alignments[0] = Alignment.buildNewAlignment(res, ids);
			...
			if (verbosity > 0) {
				...
				if (showCost) {
					long cost = Aligner.calcCost(res, ids); 
	...
	*/		
		...
		if (AlignmentMaker_SingleSequences.consistency) {
			CostMatrix.multiplyCosts((float)0.5);
			CostMatrix.increaseCosts(-consistency_subs_increase); // arbitrary number ... just trying to make w-w identities non-zero cost, and high enough to avoid rounding effects
			Aligner.multiplyGapCosts((float)0.5);
		...
		if (Polisher.polishMethod != null && 
			...
			if (Polisher.polishMethod == Polisher.PolishType.exhaust_threecut) {
				if (verbosity>0) LogWriter.stdErrLogln("First: exhaustive polishing phase");
				...
				if (verbosity>0) LogWriter.stdErrLogln("Next: three-cut polishing phase");
...
//			polisher.polish(Polisher.polishIterations);
	...
	public boolean printOutput (int[][] reorderedSeqs) {
		...
		if (verbosity>0) {
			...
			LogWriter.stdErrLogln("Input is treated as a " + (CostMatrix.isDNA ? "DNA" : "protein") + " sequence");
			...
			if (showCost) {
				long cost = Aligner.calcCost(result, ids); 
				LogWriter.stdErrLogln("Alignment cost:      " + NumberFormat.getInstance().format( cost ));
			}
			LogWriter.stdErrLogln("================================");
		}
			...
			if (Polisher.polishMethod != null ) {
				if (Polisher.polishMethod == Polisher.PolishType.exhaust_twocut) { 
					LogWriter.stdErrLogln("Exhaustive two-cut polishing was performed, making " + 
							ExhaustiveTwoCutPolisher.passCnt + " pass" + 
							(ExhaustiveTwoCutPolisher.passCnt>1?"es":"") +" over the tree");
				} else if (Polisher.polishMethod == Polisher.PolishType.random_tree_twocut) {
...
/*						
			...
			if (Aligner.useStructure) {
				LogWriter.stdErrLogln("Using structure model: " + StructureAlignment.modelType);
			}
	...
	public void initialize (int[][] seqs, String[] names) {
		this.seqs = seqs;
		this.names = names;
	...
	public void initialize (String file, String structFile) { 
		...
		SequenceFileReader seqReader = new SequenceFileReader(file, !initAlignmentProvided);		
		chars = seqReader.getSeqs();
		
		if (structFile != null)
			StructureFileReader.initialize(structFile);
		...
		initialize (SequenceConverter.convertSeqsToInts(chars), seqReader.getNames());
	}
	...
	protected Alignment[] getAlignments (int[][] seqs) {
		...
		if (initAlignmentProvided) {
			...
			alignments[0] = Alignment.buildNewAlignment(seqs, ids);
		} else {
			alignments = new Alignment[seqs.length];
			for (int i=0; i<seqs.length; i++) {
				alignments[i] = Alignment.buildNewAlignment(seqs[i], i);
			}
		}
		return alignments;
	}
...
//			else
	...
	protected Tree getTree (Alignment[] als, int currIteration) {
		if (Tree.treeType == Tree.TreeType.mst)
			return new MST_Tree(als, al, dist, currIteration, verbosity);
		else if (Tree.treeType == Tree.TreeType.entered) {
			EnteredTree.names = names;
			return new EnteredTree(als, al, dist, currIteration, verbosity);
		} else {
	...
	protected void getDistance(int currIteration) {
		Aligner pairAligner;
