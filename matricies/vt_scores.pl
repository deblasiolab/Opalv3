...
# vt_scores.pl
#
# written by Kai Kneutgen and Tobias Mueller [May 2002].
...
# WWW:   http://www.biozentrum.uni-wuerzburg.de/tobias_mueller.html
#
# Date 16.05.2002
#=========================================================================================
...
$0                =~ s!^(.*/)!!;
$pname            = $0;
$USAGE = "\n\nUSAGE: $pname <score family> <pam distance> <file name> <units in bits>\n
valid score families: VT VTML SLIM
valid pam distances in: [1 500]
valid units: 1, 2, 3, 10\n\n";
...
# Scorefactor
#-------------------------------------
if ($factor != 1 and $factor != 2 and $factor != 3 and $factor != 10)
  {
	...
	for ($zeile=0; $zeile<$anzahl_zeilen; $zeile++) {
          for ($spalte=0; $spalte<$anzahl_spalten; $spalte++) {
            if($zeile==0 || $spalte==0){
							...
							printf "%10f\t", $matrix[$zeile][$spalte];
						}
          }
          print ("\n");
	}
