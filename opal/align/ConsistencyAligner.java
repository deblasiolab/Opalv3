...
//			int[][] result = SequenceConverter.convertPathToCharAlignment(al.getPath(), A, B);
	...
	public void preprocessTree (Tree tree, float distances[][]) {
		...
		if (PairwiseAlignmentsContainer.blendMethod == PairwiseAlignmentsContainer.BlendType.symmetric ) {
			alignmentContainer = new PairwiseAlignmentContainer_symmetricBlend(tree, distances, config);
		} else if (PairwiseAlignmentsContainer.blendMethod == PairwiseAlignmentsContainer.BlendType.simple) {
			alignmentContainer = new PairwiseAlignmentContainer_simpleBlend(tree, distances, config);
		} else if (PairwiseAlignmentsContainer.blendMethod == PairwiseAlignmentsContainer.BlendType.asym_oneparam ) {
			alignmentContainer = new PairwiseAlignmentContainer_asymBlendOneParam(tree, distances, config);
		} else if (PairwiseAlignmentsContainer.blendMethod == PairwiseAlignmentsContainer.BlendType.asym_twoparam ) {
			alignmentContainer = new PairwiseAlignmentContainer_asymBlendTwoParam(tree, distances, config);
		}
	...
	protected void initialize() {
		...
		al.align();
		int[][] result = config.sc.convertPathToIntAlignment(al.getPath(), A, B);
...
//		char[][] result2 = SequenceConverter.convertPathToCharAlignment(al.getPath(), SequenceConverter.convertIntsToSeqs(alA.seqs), SequenceConverter.convertIntsToSeqs(alB.seqs));
		...
		result = config.sc.convertPathToIntAlignment (al.getPath(), alA, alB);
		...
		shapeTester = new ConsistencyShapeTester (costUpperBound, al.getD(), al.getH(), al.getV(), config);
		...
		dpRows[currRow][0].add(new ConsistencyShape(config, A, B)); // initialize entry (0,0) with the flush shape and cost 0	  
