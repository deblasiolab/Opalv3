...
public class ShapeTesterLinear extends ShapeTester {
	...
	final protected void calcGapBounds(Shape s, long[] H_V_D) {
		...
		buildBlockArrays (s, Bas, Bbs, Basc, Basc_term_left, Basc_term_right, Bbsc, Bbsc_term_left, Bbsc_term_right, Basc0, Basc0_term_left, Basc0_term_right , Bbsc0, Bbsc0_term_left, Bbsc0_term_right);
		...
		for (int block=1; block<sBlocknum; block++){
			//overhangs
			val = Bas[block] * Bbsc[block-1] * config.gamma  + Bas[block] * Bbsc_term_left[block-1] * config.leftGammaTerm() + Bas[block] * Bbsc_term_right[block-1] * config.rightGammaTerm() ; 
			Vgap += val;
			val = Bas[block] * Bbsc0[block-1] * config.gamma  + Bas[block] * Bbsc0_term_left[block-1] * config.leftGammaTerm()+ Bas[block] * Bbsc0_term_right[block-1] * config.rightGammaTerm() ;
			Hgap += val;
			Dgap += val;
			
			//underhangs
			val = block==0 ? 0 : Bbs[block] * Basc[block-1] * config.gamma  + Bbs[block] * Basc_term_left[block-1] * config.leftGammaTerm() + Bbs[block] * Basc_term_right[block-1] * config.rightGammaTerm() ; 
			Hgap += val;
			val = block==0 ? 0 : Bbs[block] * Basc0[block-1] * config.gamma  + Bbs[block] * Basc0_term_left[block-1] * config.leftGammaTerm() + Bbs[block] * Basc0_term_right[block-1] * config.rightGammaTerm() ;
			Vgap += val;
			Dgap += val;
		}
	...
	private void buildBlockArrays (Shape s, int[] Ba, int[] Bb,   
			int[] Bac, 
			int[] Bac_term_left, int[] Bac_term_right, 
			int[] Bbc,
			int[] Bbc_term_left, int[] Bbc_term_right,
			int[] Bac0, 
			int[] Bac0_term_left, int[] Bac0_term_right, 
			int[] Bbc0, 
			int[] Bbc0_term_left, int[] Bbc0_term_right)  {
			...
			int[] Ba_term_left = new int[blocknum];  // # terminal seqs in block i from A in shape s  (where "terminal" means the block  preceeds)
			int[] Ba_term_right = new int[blocknum];  // # terminal seqs in block i from A in shape s  (where "terminal" means the block follows the sequence)
