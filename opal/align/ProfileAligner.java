...
//	boolean testing = true;
	...
	public void align () {
		...
		if (isPessimistic && doReverse) { 
			...
			A = Alignment.buildNewAlignment(revA, A.seqIds, true, config, A.in);
			B = Alignment.buildNewAlignment(revB, B.seqIds, true, config, A.in);
			...
			cost2  = Aligner.calcCost(result2, A.K, B.K, idsAB, config, A.in);
		...
		long cost1 = 0;
		if (isPessimistic && doReverse) 
			cost1 = Aligner.calcCost(result1, A.K, B.K, idsAB, config, A.in);
		...
		if (cost2 < cost1) {
			if (config.useStructure) {
				resultAlignment = Alignment.buildNewAlignment(result2, ids, config,A.in);
			} else {
				resultAlignment = Alignment.buildNewAlignment(config.sc.buildReverseAlignment(result2), ids, config, A.in);
			}
		} else { 
			resultAlignment = Alignment.buildNewAlignment(result1, ids, config, A.in);
		}
	}
	...
	protected void fillTable() {
		...
		int begGapOpenCount_H, leftTermGapExtCount_H, rightTermGapExtCount_H; 
		int begGapOpenCount_V, leftTermGapExtCount_V, rightTermGapExtCount_V;
		int begGapOpenCount_D, leftTermGapExtCount_D, rightTermGapExtCount_D;
		int fA1fB1;
		long x,y,z;
		for (int i=1; i<=M; i++){
			for (int j=1; j<=N; j++){				
				...
				leftTermGapExtCount_H = B.f1[j] * (A.gapsBeforeFirst[i]) ;
				rightTermGapExtCount_H = B.f1[j] * (A.gapsAfterLast[i]+A.lastLetterCount[i]) ;

				begGapOpenCount_V = B.gapsBeforeFirst[j] * A.firstLetterCount[i];
				leftTermGapExtCount_V = A.f1[i] * (B.gapsBeforeFirst[j]) ;				
				rightTermGapExtCount_V = A.f1[i] * (B.gapsAfterLast[j]+B.lastLetterCount[j]) ;				

				begGapOpenCount_D = begGapOpenCount_H + begGapOpenCount_V;
				leftTermGapExtCount_D = B.f1[j] * (A.gapsBeforeFirst[i]) 
						+A.f1[i] * (B.gapsBeforeFirst[j]); 
				rightTermGapExtCount_D = B.f1[j] * (A.gapsAfterLast[i]) 
				...
				H[i][j] += config.lambda * (K*B.f1[j] - leftTermGapExtCount_H - rightTermGapExtCount_H) 
						+ config.leftLambdaTerm() * leftTermGapExtCount_H
						+ config.rightLambdaTerm() * rightTermGapExtCount_H;
				V[i][j] += config.lambda * (L*A.f1[i] - leftTermGapExtCount_V - rightTermGapExtCount_V) 
						+ config.leftLambdaTerm() * leftTermGapExtCount_V 
						+ config.rightLambdaTerm() * rightTermGapExtCount_V; 
...
//				all substitution letter combos
				...
				D[i][j] += config.lambda * ( (A.f0[i] * B.f1[j] + A.f1[i] * B.f0[j])-leftTermGapExtCount_D-rightTermGapExtCount_D) + 
						config.leftLambdaTerm() * leftTermGapExtCount_D + 
						config.rightLambdaTerm() * rightTermGapExtCount_D;
...
//			    LogWriter.stdOutLogln("[" + i + "," + j + "]: H = " + H[i][j] + " , V = " + V[i][j] + " , D = " + D[i][j]);
		...
		//	for (int j=1; j<=N; j++){				
				...
				if (isPessimistic) {
					...
					HIJ = x<y ? x<z?x:z  :  y<z?y:z  ;

					H_dirIJ = x<y ? x<z?Direction.horiz:Direction.diag  :  y<z?Direction.vert:Direction.diag  ;
					...
					VIJ = x<y ? x<z?x:z  :  y<z?y:z  ;

					V_dirIJ = x<y ? x<z?Direction.horiz:Direction.diag  :  y<z?Direction.vert:Direction.diag  ;
					...
					DIJ = x<y ? x<z?x:z  :  y<z?y:z  ;
					D_dirIJ = x<y ? x<z?Direction.horiz:Direction.diag  :  y<z?Direction.vert:Direction.diag  ;
				...
				HIJ += config.lambda * (K*B.f1[j] - termGapExtCountLeft_H - termGapExtCountRight_H)
						+ config.rightLambdaTerm() * termGapExtCountRight_H
						+ config.leftLambdaTerm() * termGapExtCountLeft_H;
				VIJ += config.lambda * (L*A.f1[i] - termGapExtCountLeft_V - termGapExtCountRight_V)
...
//				all substitution letter combos
				for (int m=0; m<A.chars[i].length; m++){
					for (int n=0; n<B.chars[j].length; n++){
						int s = config.cost.costs[A.chars[i][m]][B.chars[j][n]]; 
						DIJ += A.freqs[i][m]*B.freqs[j][n]*s;
					}
				}
				
				// plus extension costs				
				DIJ += config.lambda * ( (A.f0[i] * B.f1[j] + A.f1[i] * B.f0[j])-termGapExtCountLeft_D-termGapExtCountRight_D) 
						+ config.leftLambdaTerm() * termGapExtCountLeft_D
						+ config.rightLambdaTerm() * termGapExtCountRight_D;
				
				if (config.useStructure) {
					DIJ += getStructSubModifier((StructureAlignment)A, (StructureAlignment)B, i,j, config) +
							getStructGapExtModifer((StructureAlignment)A, (StructureAlignment)B, i, j, Direction.diag);
					VIJ += getStructGapExtModifer((StructureAlignment)A, (StructureAlignment)B, i, j, Direction.vert);
					HIJ += getStructGapExtModifer((StructureAlignment)A, (StructureAlignment)B, i, j, Direction.horiz);
				}		
	...
	protected boolean recover () {
		...
		while (i>0 && j>0) {			
			...
			if (Direction.horiz == dir){
				...
				int leftTermGapExtCount_H = B.f1[j] * (A.gapsBeforeFirst[i]) ;
				int rightTermGapExtCount_H = B.f1[j] * (A.gapsAfterLast[i]+A.lastLetterCount[i]) ;
				int endGapOpenCount_H_pess = B.f01[j] * (A.gapsAfterLast[i]+A.lastLetterCount[i]) ;
				int endGapOpenCount_Hv_pess = B.f1[j] * (A.gapsAfterLast[i]+A.lastLetterCount[i]);
				int endGapOpenCount_Hd_pess = B.f1[j] * A.lastLetterCount[i] + B.f01[j] * A.gapsAfterLast[i];
				int endGapOpenCount_H_opt = B.f1[j] * A.lastLetterCount[i] ; // only for V and D
								
				int base = config.lambda * (K*B.f1[j] - leftTermGapExtCount_H - rightTermGapExtCount_H) 
						+ config.leftLambdaTerm() * leftTermGapExtCount_H
						+ config.rightLambdaTerm() * rightTermGapExtCount_H;
				if (config.useStructure) {
				...
				int D_add_value;
				if(isPessimistic){
					H_add_value = config.gamma * ( (K * B.f01[j])-(begGapOpenCount_H+endGapOpenCount_H_pess))
							+ config.leftGammaTerm()*(begGapOpenCount_H)
							+ config.rightGammaTerm()*(endGapOpenCount_H_pess);
					V_add_value = config.gamma * ( (K * B.f1[j])-(begGapOpenCount_H+endGapOpenCount_Hv_pess) ) 
							+ config.leftGammaTerm()*(begGapOpenCount_H)
							+ config.rightGammaTerm()*(endGapOpenCount_Hv_pess);	
					D_add_value = config.gamma * ( (A.f1[i]*B.f1[j] + A.f0[i]*B.f01[j])-(begGapOpenCount_H+endGapOpenCount_Hd_pess)) 
							+ config.leftGammaTerm()*(begGapOpenCount_H)
							+ config.rightGammaTerm()*(endGapOpenCount_Hd_pess);
				}else{
					H_add_value = config.leftGammaTerm()*begGapOpenCount_H;
					V_add_value = config.gamma * (fA1fB1 - (begGapOpenCount_H+endGapOpenCount_H_opt)) 
							+ config.leftGammaTerm()*(begGapOpenCount_H)
							+ config.rightGammaTerm()*(endGapOpenCount_H_opt);
					D_add_value =  config.gamma * (fA1fB1 - (begGapOpenCount_H+endGapOpenCount_H_opt)) 
							+ config.leftGammaTerm()*(begGapOpenCount_H)
							+ config.rightGammaTerm()*(endGapOpenCount_H_opt);
				}
				if ( cost == H[i][j-1] + base + x + H_add_value) {				
					nextdir = Direction.horiz;
					if (testing) LogWriter.stdOutLogln("hh(" + i + "," + j+ "): " + (cost));
				} else if ( cost == V[i][j-1] + base + y + V_add_value) {
					nextdir = Direction.vert;
					if (testing) LogWriter.stdOutLogln("vh(" + i + "," + j+ "): " + (cost));
				} else if ( cost == D[i][j-1] + base + z + D_add_value) {				
				...
				int leftTermGapExtCount_V = A.f1[i] * (B.gapsBeforeFirst[j]) ;	
				int rightTermGapExtCount_V = A.f1[i] * (B.gapsAfterLast[j]+B.lastLetterCount[j]) ;				
				int endGapOpenCount_V_pess = A.f01[i] * (B.gapsAfterLast[j]+B.lastLetterCount[j]) ;
				int endGapOpenCount_Vh_pess = A.f1[i] * (B.gapsAfterLast[j]+B.lastLetterCount[j]);
				int endGapOpenCount_Vd_pess = A.f1[i] * B.lastLetterCount[j] +  A.f01[i] * B.gapsAfterLast[j];
				int endGapOpenCount_V_opt = A.f1[i] * B.lastLetterCount[j] ; // only for H and D
				
				int base = config.lambda * (L*A.f1[i] - leftTermGapExtCount_V - rightTermGapExtCount_V)
						+ config.leftLambdaTerm() * leftTermGapExtCount_V
						+ config.rightLambdaTerm() * rightTermGapExtCount_V;
				if (config.useStructure) {
				...
				int D_add_value;
				if(isPessimistic){
					H_add_value = config.gamma*( (L * A.f1[i])-(begGapOpenCount_V+endGapOpenCount_Vh_pess) ) 
							+ config.leftGammaTerm()*(begGapOpenCount_V)
							+ config.rightGammaTerm()*(endGapOpenCount_Vh_pess);
					V_add_value = config.gamma*( (L * A.f01[i])-(begGapOpenCount_V+endGapOpenCount_V_pess) ) 
							+ config.leftGammaTerm()*(begGapOpenCount_V)
							+ config.rightGammaTerm()*(endGapOpenCount_V_pess);
					D_add_value = config.gamma*( (fA1fB1 + A.f01[i]*B.f0[j])-(begGapOpenCount_V+endGapOpenCount_Vd_pess) ) 
							+ config.leftGammaTerm()*(begGapOpenCount_V)
							+ config.rightGammaTerm()*(endGapOpenCount_Vd_pess);
				}else{
					H_add_value =  config.gamma * (fA1fB1 - (begGapOpenCount_V+endGapOpenCount_V_opt)) 
							+ config.leftGammaTerm()*(begGapOpenCount_V)
							+ config.rightGammaTerm()*(endGapOpenCount_V_opt);
					V_add_value = config.leftGammaTerm()*begGapOpenCount_V;
					D_add_value = config.gamma * (fA1fB1 - (begGapOpenCount_V+endGapOpenCount_V_opt)) 
							+ config.leftGammaTerm()*(begGapOpenCount_V)
							+ config.rightGammaTerm()*(endGapOpenCount_V_opt);
				}
				if ( cost == H[i-1][j] + base + x +  H_add_value) {
					nextdir = Direction.horiz;
					if (testing) LogWriter.stdOutLogln("hv(" + i + "," + j+ "): " + (cost));
				} else if ( cost == V[i-1][j] + base + y + V_add_value) {				
					nextdir = Direction.vert;
					if (testing) LogWriter.stdOutLogln("vv(" + i + "," + j+ "): " + (cost));
			 	} else if ( cost == D[i-1][j] + base + z + D_add_value) {					
					nextdir = Direction.diag;
				...
				int leftTermGapExtCount_D = B.f1[j] * (A.gapsBeforeFirst[i]) 
						+ A.f1[i] * (B.gapsBeforeFirst[j]); 
				int rightTermGapExtCount_D = B.f1[j] * (A.gapsAfterLast[i]) 
						+ A.f1[i] * (B.gapsAfterLast[j]); 
				...
				int base = config.lambda * ( (A.f0[i] * B.f1[j] + A.f1[i] * B.f0[j])-leftTermGapExtCount_D-rightTermGapExtCount_D) 
						+ config.leftLambdaTerm() * leftTermGapExtCount_D
						+ config.rightLambdaTerm() * rightTermGapExtCount_D;
				...
				int D_add_value;
				if(isPessimistic){
					H_add_value = config.gamma * ((A.f1[i]*B.f0[j] + A.f0[i]*B.f01[j])-(begGapOpenCount_D+endGapOpenCount_Dh_pess)) 
							+ config.leftGammaTerm()*(begGapOpenCount_D)
							+ config.rightGammaTerm()*(endGapOpenCount_Dh_pess);
					V_add_value = config.gamma * ( (B.f1[j]*A.f0[i] + B.f0[j]*A.f01[i])-(begGapOpenCount_D+endGapOpenCount_Dv_pess)) 
							+ config.leftGammaTerm()*(begGapOpenCount_D)
							+ config.rightGammaTerm()*(endGapOpenCount_Dv_pess);
					D_add_value = config.gamma * ( (A.f1[i]*B.f10[j] + A.f10[i]*B.f1[j] + A.f00[i]*B.f01[j] + A.f01[i]*B.f00[j])-(begGapOpenCount_D+endGapOpenCount_Dd_pess)) 
							+ config.leftGammaTerm()*(begGapOpenCount_D) 
							+ config.rightGammaTerm()*(endGapOpenCount_Dd_pess) ;
					
				}else{
					H_add_value = config.gamma*(A.f1[i]*B.f10[j] + vTermGapOpens_notFirstCol - (begGapOpenCount_V+endGapOpenCount_Dh_opt)) 
							+ config.leftGammaTerm()*(begGapOpenCount_D)
							+ config.rightGammaTerm()*(endGapOpenCount_Dh_opt);
					V_add_value = config.gamma*(B.f1[j]*A.f10[i] + hTermGapOpens_notFirstCol - (begGapOpenCount_H+endGapOpenCount_Dv_opt)) 
							+ config.leftGammaTerm()*(begGapOpenCount_D)
							+ config.rightGammaTerm()*(endGapOpenCount_Dv_opt);
					D_add_value = config.gamma*( A.f1[i]*B.f10[j] + A.f10[i]*B.f1[j] + vTermGapOpens_notFirstCol + hTermGapOpens_notFirstCol- (begGapOpenCount_D+endGapOpenCount_Dd_opt)) 
							+ config.leftGammaTerm()*(begGapOpenCount_D)
							+ config.rightGammaTerm()*(endGapOpenCount_Dd_opt);
				...
				if ( cost == H[i-1][j-1] + base + x + H_add_value) {				
					nextdir = Direction.horiz;
					if (testing) LogWriter.stdOutLogln("hd(" + i + "," + j+ "): " + (cost));
				} else if ( cost == V[i-1][j-1] + base + y + V_add_value) {				
					nextdir = Direction.vert;		
					if (testing) LogWriter.stdOutLogln("vd(" + i + "," + j+ "): " + (cost));
				} else if ( cost == D[i-1][j-1] + base + z + D_add_value) {									 
					nextdir = Direction.diag;
